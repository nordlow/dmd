...
int[] assignSlice(int from, int to, int[] stuff)
{
    int[] slice;
    slice.length = to + 4;
...
static assert(makeS2Max.u == ulong.max);
static assert(makeS2Max.l == long.max);

static assert(makeS2Min.u == ulong.min);
static assert(makeS2Min.l == long.min);
